...
exports.searchEmployeeTasks = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ matchedTasks: tasks });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};

exports.searchEmployeeArchivedTasks = async (req, res) => {
    const { email, searchTerm } = req.query;
    const searchPattern = `%${searchTerm}%`;
    ...
    try {
        ...
        const [tasks] = await db.query(taskQuery, [email, searchPattern]);
        res.status(200).json({ matchedArchivedTasks: tasks });
    } catch (error) {
        res.status(500).json({ error: "خطأ في جلب المهام المؤرشفة: " + error.message });
    }
};